...
```{r visualisation}
#source data for visualisation is taken from preprocessed data folder, not after expert check
visualise_clean <- function(inter_result_item){
  ...
  plot_path <- file.path(ROOT, 
                         "plots", 
                         paste0(participant, "_", text, ".jpeg"))
  # plot(clean_text$x_coordinate, 
  #      1-clean_text$y_coordinate,
  #      type = "b",
  #      main = paste0("Participant ", participant, ", text ", text),
  #      xlab = "X",
  #      ylab = "Y")
  ggplot(data = clean_text,
    ...
    xlab("X") +
    ylab("Y") + #add 1-y for y axis to reflect actual coordinates
    theme_bw()
  ggsave(plot_path)
}
lapply(inter_result, visualise_clean)
```
