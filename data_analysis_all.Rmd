...
```{r}
ROOT <- "G:/repos/EyeTracking" #нужно заменить на путь где будет лежать скрипт и результаты выполнения
...
process_text <- function(text_n, text_data, meta_data){
  ...
  for(i in 1:(nrow(output)-1)){
    ...
    if(!is.na(output$marker_type[i]) & output$marker_type[i] == "Saccade" & !is.na(output$saccade_type[i]) & output$saccade_type[i] == "Regressive" & output$saccade_value[i] > 10){
      ...
      while(!is.na(output$marker_type[i+k]) & output$marker_type[i+k] != "Saccade") 
...
selected_participants <- all_participants[!all_participants %in% c("P16", "P17", "P19", missing_data)]
